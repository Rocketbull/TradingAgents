...
from activeportfolio.dataflows.market_data_store import find_history_parquet
...
def _load_symbol_lines(path: str | Path) -> list[str]:
    rows = [
...
def get_index_stocks(
    ...
    *,
    ...
    if isinstance(index, (list, tuple, set, pd.Index, np.ndarray)):
        rows = [str(symbol).strip().upper() for symbol in index if str(symbol).strip()]
        if not rows:
            raise ValueError("index universe is empty")
        return list(dict.fromkeys(rows))

    label = str(index or DEFAULT_US_BENCHMARK).strip().upper()
    if not label:
        label = DEFAULT_US_BENCHMARK

    if label in {"SPY", "IVV", "VOO", "SP500", "S&P500", "S&P 500"}:
        path = Path(symbol_file) if symbol_file is not None else DEFAULT_SP500_SYMBOL_FILE
        if not path.exists():
            raise FileNotFoundError(f"S&P 500 symbol file not found: {path}")
        symbols = [symbol for symbol in _load_symbol_lines(path) if symbol != label]
        if not symbols:
            raise ValueError(f"No S&P 500 constituents found in {path}")
        return symbols

    if label in {"CSI300", "000300", "000300.SH", "000300.XSHG"}:
        path = Path(csi300_file)
        if not path.exists():
            raise FileNotFoundError(f"CSI300 constituent file not found: {path}")
        frame = pd.read_csv(path)
        if "symbol" not in frame.columns:
            raise ValueError(f"CSI300 constituent file missing 'symbol' column: {path}")
        symbols = [
...
def _read_daily_history(
    ...
    *,
    ...
    if "Date" not in history.columns:
        raise ValueError(f"Unexpected history schema for {normalized}: missing Date column")

    history["Date"] = pd.to_datetime(history["Date"]).dt.normalize()
    history = history[history["Date"] <= end_ts].sort_values("Date")
    if history.empty:
        raise ValueError(
            f"No local history rows for {normalized} on or before {end_ts.strftime('%Y-%m-%d')}"
        )
...
def _build_requested_fields(
    ...
    *,
    ...
    base = pd.DataFrame({"date": pd.to_datetime(history["Date"]).dt.normalize()})
    ...
    for field in fields:
        if field not in SUPPORTED_PRICE_FIELDS:
            raise ValueError(f"Unsupported field '{field}'. Allowed: {sorted(SUPPORTED_PRICE_FIELDS)}")

        if field == "open":
            base[field] = pd.to_numeric(history["Open"], errors="coerce")
        elif field == "close":
            base[field] = close
        elif field == "low":
            base[field] = pd.to_numeric(history["Low"], errors="coerce")
        elif field == "high":
            base[field] = pd.to_numeric(history["High"], errors="coerce")
        elif field == "volume":
            base[field] = volume
        elif field == "prev_close":
            base[field] = close.shift(1).fillna(close)
        elif field == "turnover":
            base[field] = close * volume
        elif field == "avg_price":
            missing_fields.append(field)
            base[field] = np.nan

    return base.set_index("date").tail(int(count)), missing_fields
...
def get_price(
    ...
    *,
    ...
    """Compatibility wrapper backed by locally downloaded daily yfinance parquet data."""
    del start_date, skip_paused, fq, is_panel  # Legacy args kept for call compatibility.

    if frequency != "1d":
        raise ValueError("Only daily frequency ('1d') is supported by local yfinance parquet data")

    requested_fields = list(fields or ["open", "close", "low", "high", "volume"])
    if not requested_fields:
        raise ValueError("fields must not be empty")

    if isinstance(security, str):
        history = _read_daily_history(security, end_date=end_date, count=count, root_dir=root_dir)
        frame, missing_fields = _build_requested_fields(history, requested_fields, count=count)
        frame.attrs["missing_fields"] = missing_fields
        return frame

    symbols = [str(symbol).strip().upper() for symbol in security if str(symbol).strip()]
    if not symbols:
    ...
    for symbol in symbols:
        history = _read_daily_history(symbol, end_date=end_date, count=count, root_dir=root_dir)
        frame, missing = _build_requested_fields(history, requested_fields, count=count)
        per_symbol[symbol] = frame
        missing_fields.update(missing)

    panel = pd.concat(per_symbol, axis=1)
    stacked = pd.concat(
        {
            field: panel.xs(field, axis=1, level=1)
            for field in requested_fields
        },
        axis=0,
    )
    stacked.index.names = ["field", "date"]
    stacked.attrs["missing_fields"] = sorted(missing_fields)
    return stacked


def _alpha_methods_using(attr_name: str) -> list[str]:
    methods: list[str] = []
    for name, fn in GTJA_191.__dict__.items():
        if not name.startswith("alpha_") or not callable(fn):
            continue
        try:
            source = inspect.getsource(fn)
        except (OSError, TypeError):
            continue
        if f"self.{attr_name}" in source:
            methods.append(name)
...
class GTJA_191:
    ...
    def __init__(
        self,
        end_date,
        index=DEFAULT_US_BENCHMARK,
        *,
        benchmark: str | None = None,
        symbol_file: str | Path | None = None,
        root_dir: str | Path = "data/market",
    ):
        self.end_date = _as_timestamp(end_date)
        self.index = index
        self.benchmark_symbol = str(benchmark or index or DEFAULT_US_BENCHMARK).strip().upper()
        security = get_index_stocks(index, symbol_file=symbol_file)
        price = get_price(
            security,
            None,
            self.end_date,
        ...
        benchmark_price = get_price(
            ...
            self.end_date,
            "1d",
            ["open", "close", "low", "high", "avg_price", "prev_close", "volume"],
            False,
            None,
            250,
            is_panel=1,
        ...
        price_fields = {
            field: price.xs(field, axis=0, level=0).sort_index()
            for field in ["open", "close", "low", "high", "avg_price", "prev_close", "volume", "turnover"]
        }
        core_symbols = price_fields["close"].columns
        for field in ["open", "low", "high", "prev_close", "volume", "turnover"]:
            core_symbols = core_symbols.intersection(price_fields[field].columns)

        self.open_price = price_fields["open"].loc[:, core_symbols]
        self.close = price_fields["close"].loc[:, core_symbols]
        self.low = price_fields["low"].loc[:, core_symbols]
        self.high = price_fields["high"].loc[:, core_symbols]
        self.prev_close = price_fields["prev_close"].loc[:, core_symbols]
        self.volume = price_fields["volume"].loc[:, core_symbols]
        self.amount = price_fields["turnover"].loc[:, core_symbols]

        missing_fields = sorted(
            set(price.attrs.get("missing_fields", [])) | set(benchmark_price.attrs.get("missing_fields", []))
        )
        self.missing_data_fields = tuple(missing_fields)
        self.avg_price_affected_alphas = _alpha_methods_using("avg_price") if "avg_price" in missing_fields else []
        self.data_warnings: list[str] = []

        if "avg_price" in missing_fields:
            self.avg_price = pd.DataFrame(np.nan, index=self.close.index, columns=core_symbols)
            self.data_warnings.append(
                "avg_price is unavailable from daily yfinance parquet data; methods in "
                "avg_price_affected_alphas will return incomplete outputs until a VWAP/avg_price source is added."
            )
        else:
            self.avg_price = price_fields["avg_price"].loc[:, core_symbols]
    ...
    #     
        benchmark = pd.DataFrame(
            {
                "open": self.benchmark_open_price.tail(50),
                "close": self.benchmark_close_price.tail(50),
            }
        )
